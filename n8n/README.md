...
3. `Run GE Prediction Script` 노드의 명령어 경로가 현재 프로젝트 경로와 맞는지 확인합니다.
4. (선택) Google Sheets 노드에서 Spreadsheet ID와 Credential을 입력합니다.
5. **Discord Webhook URL 발급 후 입력합니다 (아래 참고).**
6. 비활성화된 노드를 활성화합니다.
...
1. Discord에서 알림 받을 **채널** → ⚙️ 채널 설정 → **연동** 탭
2. **웹후크 만들기** 클릭 → 이름 설정 → **웹후크 URL 복사**
3. n8n의 `Send Discord Message` 노드 열기
4. **URL** 항목에 복사한 Webhook URL 붙여넣기
5. 노드 `disabled` 해제 후 저장

발송 메시지 형식 (Discord 마크다운):
...
🔍 RSI(14): 47.97  |  MACD: -4.60
📉 변동성(20일): 3.20%  |  MA20: $295.01

🤖 모델: sklearn MLP Dense Regression
